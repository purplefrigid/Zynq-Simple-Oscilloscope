...
void wave_handle(short BufferPtr[],short handle1[],int times,int width)
{
	int i,j,k;
	int abs;
	short handle[width*times-times+1];
	//short handle1[width];
	if(times>=1){
		for(i=0 ; i<width ; i++)
		{
			handle[i*times]=BufferPtr[i];
			for(j=i*times;j<(i+1)*times;j++)
			{
				for(k=1;k<times;k++)
				handle[j]=k/times*(BufferPtr[i+1]-BufferPtr[i])+BufferPtr[i];
			}
		}
		for(i=0; i<width; i++)
		{
			handle1[i]=handle[width*times-times-width+i];
		//handle1[i]=BufferPtr[i];
		}
	}
	else
	{
		abs=-times;
		for(j=0; j<abs;j++){
			for(i=0 ; i<width/abs ; i++)
			{
				handle1[i+j*width/abs]=BufferPtr[abs*i];
			}
		}
	}
}
